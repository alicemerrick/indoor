...
function drawVis() {
    ...
    y = d3.scaleLinear()
      	.domain([d3.min(dataset, function(x) { return x.temp; }),
          d3.max(dataset, function(x) { return x.temp; })])
      	.range([height, 0]);
...
function tick() {
    ...
    if (curPos < dataset.length) {
      ...
      d3.select('#humidity').html(
        Math.round(dataset[curPos].humidity*1)/1 +"% Humidity");
      d3.select('#temperature').html(
        Math.round(dataset[curPos].temp*1)/1 +"°F");
    }
    ...
    d3.active(this)
      ...
      now = new Date();
      var format = d3.timeFormat("%a %x %I:%M %p")
      d3.select('#time').html(format(now));
